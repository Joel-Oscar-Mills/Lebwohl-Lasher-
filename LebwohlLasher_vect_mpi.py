...
def main(program, STEPS, NMAX, Ts, pflag):
    ...
    # Check if numworkers is within range - quit if not
        ...
        #block_energy(NMAX,angles,energies,0)
        #block_energy(NMAX,angles,energies,1)
        angles_init = angles
        ...
        temp_E = np.zeros(STEPS+1)
        temp_Q = np.zeros((STEPS+1,2,2))
        temp_R = np.zeros(STEPS+1)
        ...
        for i in range(1,numworkers+1):
            ...
            comm.Recv([temp_E[1:],STEPS,MPI.DOUBLE], source=i, tag=DONE)
            comm.Recv([temp_Q[1:],STEPS*4,MPI.DOUBLE], source=i, tag=DONE)
            comm.Recv([temp_R[1:],STEPS,MPI.DOUBLE], source=i, tag=DONE)
            E += temp_E
            Q += temp_Q
            R += temp_R
    ...
    #********* workers code ************/
        ...
        comm.Send([E[1:],STEPS,MPI.DOUBLE], dest=MASTER, tag=DONE)
        comm.Send([Q[1:],STEPS*4,MPI.DOUBLE], dest=MASTER, tag=DONE)
        comm.Send([R[1:],STEPS,MPI.DOUBLE], dest=MASTER, tag=DONE)
